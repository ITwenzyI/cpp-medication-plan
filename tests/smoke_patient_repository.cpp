...
int main() {
    ...
    expect(!all_patients.value().empty(), "getAllPatients should return at least one patient after create");
    expect(all_patients.value().size() == 1, "expected exactly one patient in empty test database");
    expect(
        all_patients.value()[0].id == created_patient.value().id, "expected stored patient id to equal created.id");
    ...
    expect(found_patient.value().birth_date == created_patient.value().birth_date, "birth_date should match");
    expect(found_patient.value().nationality == created_patient.value().nationality, "nationality should match");

    // ======= UPDATE PATIENT NAME ======

    // Update Patient Name
    auto update_name_result = patient_repository.updatePatientName(created_patient.value().id, "Bjarne Stroustrup");
    ...
    expect(update_name_same_value_result.isOk(), "updatePatientName with same value should still succeed");
    ...
    auto patient_after_birthdate_update = patient_repository.findPatientById(created_patient.value().id);
    expect(patient_after_birthdate_update.isOk(), "find after update should succeed");
    expect(patient_after_birthdate_update.value().birth_date == "1999-06-06", "birthdate should be updated");
    ...
    expect(update_birthdate_invalid_format_result.error().code == common::result::ErrorCode::InvalidArgument,
        "invalid birth_date should return InvalidArgument");

    // ======= UPDATE PATIENT NATIONALITY ======

    // Update Patient Nationality
    auto update_nationality_result = patient_repository.updatePatientNationality(created_patient.value().id, "US");
    ...
    auto patient_after_nationality_update = patient_repository.findPatientById(created_patient.value().id);
    ...
    auto clear_nationality_result = patient_repository.updatePatientNationality(created_patient.value().id, "");
    expect(clear_nationality_result.isOk(), "clearing nationality should succeed");

    auto patient_after_clearing_nationality = patient_repository.findPatientById(created_patient.value().id);
    expect(patient_after_clearing_nationality.isOk(), "find after clear nationality should succeed");
    ...
    auto delete_missing_patient_result = patient_repository.deletePatientById(created_patient.value().id);
